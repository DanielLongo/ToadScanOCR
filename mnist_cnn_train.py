...
def train():
    ...
    num_labels = emnist_data.NUM_LABELS

    # Prepare mnist data
    # train_total_data, train_size, validation_data, validation_labels, test_data, test_labels = mnist_data.prepare_MNIST_data(False)
    train_total_data, train_size, validation_data, validation_labels, test_data, test_labels = emnist_data.prepare_EMNIST_data(False)
    ...
    y_ = tf.placeholder(tf.float32, [None, 47]) #answer
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            if validation_accuracy > max_acc:
                ...
                # tf.saved_model.s
                # tf.compat.v1.saved_model.simple_save(sess, "./model", inputs={"x": x}, outputs={"y": y})
                print("saved")
